#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <unistd.h>
#include <sys/types.h> 
#ifdef __unix__
#include <sys/socket.h>
#include <netinet/in.h>
#endif
#include "llsim.h"

#define sp_printf(a...)						\
	do {							\
		llsim_printf("sp: clock %d: ", llsim->clock);	\
		llsim_printf(a);				\
	} while (0)

int nr_simulated_instructions = 0;
FILE *inst_trace_fp = NULL, *cycle_trace_fp = NULL;

typedef struct sp_registers_s {
	// 6 32 bit registers (r[0], r[1] don't exist)
	int r[8];

	// 16 bit program counter
	int pc;

	// 32 bit instruction
	int inst;

	// 5 bit opcode
	int opcode;

	// 3 bit destination register index
	int dst;

	// 3 bit source #0 register index
	int src0;

	// 3 bit source #1 register index
	int src1;

	// 32 bit alu #0 operand
	int alu0;

	// 32 bit alu #1 operand
	int alu1;

	// 32 bit alu output
	int aluout;

	// 32 bit immediate field (original 16 bit sign extended)
	int immediate;

	// 32 bit cycle counter
	int cycle_counter;

	// 3 bit control state machine state register
	int ctl_state;

	// control states
	#define CTL_STATE_IDLE		0
	#define CTL_STATE_FETCH0	1
	#define CTL_STATE_FETCH1	2
	#define CTL_STATE_DEC0		3
	#define CTL_STATE_DEC1		4
	#define CTL_STATE_EXEC0		5
	#define CTL_STATE_EXEC1		6

	// <<< DMA: add DMA state & parameters
    int dma_state;    // 0=IDLE, 1=READ, 2=WRITE
    int dma_src;      // source base address
    int dma_dst;      // destination base address
    int dma_len;      // total words to copy
    int dma_count;    // how many words copied so far
} sp_registers_t;


/*
 * Master structure
 */
typedef struct sp_s {
	// local sram
#define SP_SRAM_HEIGHT	64 * 1024
	llsim_memory_t *sram;

	unsigned int memory_image[SP_SRAM_HEIGHT];
	int memory_image_size;

	sp_registers_t *spro, *sprn;
	
	int start;
} sp_t;

static void sp_reset(sp_t *sp)
{
	sp_registers_t *sprn = sp->sprn;

	memset(sprn, 0, sizeof(*sprn));
}

/*
 * opcodes
 */
#define ADD 0
#define SUB 1
#define LSF 2
#define RSF 3
#define AND 4
#define OR  5
#define XOR 6
#define LHI 7
#define LD 8
#define ST 9
///////DMA ///////////// 
#define DMA_START   10   // R[src0] = src addr, R[src1] = dst addr, imm = length
#define DMA_STATUS  11   // R[dst] := (dma_state != 0)
///////////////////////
#define JLT 16
#define JLE 17
#define JEQ 18
#define JNE 19
#define JIN 20
#define HLT 24

static char opcode_name[32][4] = {"ADD", "SUB", "LSF", "RSF", "AND", "OR", "XOR", "LHI",
				 "LD", "ST", "U", "U", "U", "U", "U", "U",
				 "JLT", "JLE", "JEQ", "JNE", "JIN", "U", "U", "U",
				 "HLT", "U", "U", "U", "U", "U", "U", "U"};

static void dump_sram(sp_t *sp)
{
	FILE *fp;
	int i;

	fp = fopen("sram_out.txt", "w");
	if (fp == NULL) {
                printf("couldn't open file sram_out.txt\n");
                exit(1);
	}
	for (i = 0; i < SP_SRAM_HEIGHT; i++)
		fprintf(fp, "%08x\n", llsim_mem_extract(sp->sram, i, 31, 0));
	fclose(fp);
}


static void sp_ctl(sp_t *sp)
{
	sp_registers_t *spro = sp->spro;
	sp_registers_t *sprn = sp->sprn;
	int i;

	// sp_ctl

	fprintf(cycle_trace_fp, "cycle %d\n", spro->cycle_counter);
	for (i = 2; i <= 7; i++){
	fprintf(cycle_trace_fp,"r%d       :%08x || ", i, spro->r[i]);
	}
	fprintf(cycle_trace_fp,"\n");
	fprintf(cycle_trace_fp, "pc       :%08x || ", spro->pc);
	fprintf(cycle_trace_fp, "inst     :%08x || ", spro->inst);
	fprintf(cycle_trace_fp, "opcode   :%08x || ", spro->opcode);
	fprintf(cycle_trace_fp, "dst      :%08x || ", spro->dst);
	fprintf(cycle_trace_fp, "src0     :%08x || ", spro->src0);
	fprintf(cycle_trace_fp, "src1     :%08x || ", spro->src1);
	fprintf(cycle_trace_fp, "immediate:%08x \n", spro->immediate);
	fprintf(cycle_trace_fp, "alu0     :%08x || ", spro->alu0);
	fprintf(cycle_trace_fp, "alu1     :%08x || ", spro->alu1);
	fprintf(cycle_trace_fp, "aluout   :%08x\n", spro->aluout);
	fprintf(cycle_trace_fp, "cycle_counter %08x\n", spro->cycle_counter);
	fprintf(cycle_trace_fp, "ctl_state %08x\n\n", spro->ctl_state);

	sprn->cycle_counter = spro->cycle_counter + 1;

	switch (spro->ctl_state) {
	case CTL_STATE_IDLE:{
		sprn->pc = 0;
		if (sp->start)
			sprn->ctl_state = CTL_STATE_FETCH0;
		break;
	}
	case CTL_STATE_FETCH0:{
		llsim_mem_read(sp->sram , spro->pc); // reads the memory at the address PC in the given sram
		sprn->ctl_state = CTL_STATE_FETCH1; //jumps to FETCH1 to sample memory 
		break;
	}
	case CTL_STATE_FETCH1:{
		sprn->inst = llsim_mem_extract_dataout(sp->sram, 31, 0); //extracts the read memory into inst
		sprn->ctl_state = CTL_STATE_DEC0; // moves to the decode0 stage 
		break;
	}	
	case CTL_STATE_DEC0:{
		int temp =spro->inst;
		sprn->opcode = (temp>>25) & 0X1F; //Extracts a 5 bit opcode from bits 30-25
		sprn->dst    = (temp>>22) & 0X7;  //Extracts a 3 bit destination register from bits 24-22
		sprn->src0   = (temp>>19) & 0X7;  //Extracts a 3 bit source0 register from bits 21-19
		sprn->src1   = (temp>>16) & 0X7;  //Extracts a 3 bit source1 register from bits 18-16
		int imm      = temp & 0X0000ffff; //extract 16 bit immediate 
		if (imm & 0X8000) 				  //sign-extend the 16 bit immediate to 32 bits
			imm |= 0XFFFF0000;
		sprn->immediate = imm;
		sprn->ctl_state = CTL_STATE_DEC1; //  move to decode1 stage
		break;
	}
	case CTL_STATE_DEC1:
	
	    // Prepare ALU operand 0.
	    // Use a switch-case to select the appropriate source for alu0 based on sprn->src0.
	    switch (spro->src0) {
    	    case 0:
        	    // When source 0 is 0, the operand is a constant zero.
            	sprn->alu0 = 0;
            	break;
	        case 1:
    	        // When source 0 is 1, the operand comes from the sign-extended immediate field.
        	    sprn->alu0 = spro->immediate;
            	break;
	        default:
    	        // For any other value (e.g., 2 through 7),
        	    // use the value from the corresponding register in the old register file.
            	sprn->alu0 = spro->r[spro->src0];
	            break;
    	}

	    // Prepare ALU operand 1.
    	// Use a similar switch-case structure to select the source for alu1 based on sprn->src1.
	    switch (spro->src1) {
    	    case 0:
        	    // When source 1 is 0, use the constant zero.
            	sprn->alu1 = 0;
            	break;
	        case 1:
    	        // When source 1 is 1, use the immediate value.
        	    sprn->alu1 = spro->immediate;
            	break;
	        default:
    	        // For any other value, fetch the operand from the old register (spro).
        	    sprn->alu1 = spro->r[spro->src1];
            	break;
	    }
		if (spro->opcode == LHI) {
			sprn->alu0 = 0;             // Force the lower-half operand to zero.
			sprn->alu1 = spro->immediate; // Use the immediate for the high half.
		}
		

    	// Once both operands are prepared, transition to the execution phase.
    	sprn->ctl_state = CTL_STATE_EXEC0;
    	break;


	case CTL_STATE_EXEC0: {
		switch (spro->opcode) {
			// --- Arithmetic and Logical Instructions ---
			case ADD: 
				sprn->aluout = spro->alu0 + spro->alu1;
				break;
			case SUB:
				sprn->aluout = spro->alu0 - spro->alu1;
				break;
			case LSF:
				sprn->aluout = spro->alu0 << spro->alu1;
				break;
			case RSF:
				sprn->aluout = spro->alu0 >> spro->alu1;
				break;
			case AND:
				sprn->aluout = spro->alu0 & spro->alu1;
				break;
			case OR:
				sprn->aluout = spro->alu0 | spro->alu1;
				break;
			case XOR:
				sprn->aluout = spro->alu0 ^ spro->alu1;
				break;
			case LHI:
				// LHI: Combine the immediate (in alu1) with the lower 16 bits of alu0.
				sprn->aluout = (spro->alu1 << 16) | (spro->alu0 & 0xFFFF);
				break;
					
			// --- Conditional (Jump/Compare) Instructions ---
			case JLT:
				sprn->aluout = (spro->alu0 < spro->alu1) ? 1 : 0;
				break;
			case JLE:
				sprn->aluout = (spro->alu0 <= spro->alu1) ? 1 : 0;
				break;
			case JEQ:
				sprn->aluout = (spro->alu0 == spro->alu1) ? 1 : 0;
				break;
			case JNE:
				sprn->aluout = (spro->alu0 != spro->alu1) ? 1 : 0;
				break;
					
			// --- Memory/Control Instructions ---
			case LD:
				llsim_mem_read(sp->sram, spro->alu1); // reads memory of adress of register src1
				sprn->aluout = 0;  // Not used in EXEC0.
				break;
			case ST:
				sprn->aluout = spro->alu1;  // Compute effective address. value to be saved is the the value of register
				break;
			default:
				sprn->aluout = 0;
				break;
		}
		// Transition to the next execution state (EXEC1).
		sprn->ctl_state = CTL_STATE_EXEC1;
		break;
	}

    case CTL_STATE_EXEC1: {
        switch (spro->opcode) {
            
			// <<< DMA_START: capture parameters and kick off FSM
            case DMA_START:
                sprn->dma_src    = spro->r[spro->src0];
                sprn->dma_dst    = spro->r[spro->src1];
                sprn->dma_len    = spro->immediate;
                sprn->dma_count  = 0;
                sprn->dma_state  = 1;
                sprn->pc = (spro->pc + 1) & 0xFFFF;
                sprn->ctl_state = CTL_STATE_FETCH0;
                break;

            // <<< DMA_STATUS: write busy flag into dst reg
            case DMA_STATUS:
                if (spro->dst != 0)
                    sprn->r[spro->dst] = (spro->dma_state != 0);
                sprn->pc = (spro->pc + 1) & 0xFFFF;
                sprn->ctl_state = CTL_STATE_FETCH0;
                break;			
			
			// --- Arithmetic / Logical Instructions ---
            case ADD: 
				if (spro->dst != 0)
					sprn->r[spro->dst] = sprn->aluout;
				sprn->pc = (spro->pc + 1) & 0xFFFF; 
            case SUB:
			    if (spro->dst != 0)
                	sprn->r[spro->dst] = spro->aluout;
            	sprn->pc = (spro->pc + 1) & 0xFFFF; 
            case LSF:
				if (spro->dst != 0)
					sprn->r[spro->dst] = sprn->aluout;
				sprn->pc = (spro->pc + 1) & 0xFFFF; 
            case RSF:
				if (spro->dst != 0)
					sprn->r[spro->dst] = sprn->aluout;
				sprn->pc = (spro->pc + 1) & 0xFFFF; 
            case AND:
				if (spro->dst != 0)
					sprn->r[spro->dst] = sprn->aluout;
				sprn->pc = (spro->pc + 1) & 0xFFFF; 
            case OR:
				if (spro->dst != 0)
					sprn->r[spro->dst] = sprn->aluout;
				sprn->pc = (spro->pc + 1) & 0xFFFF; 
            case XOR:
				if (spro->dst != 0)
					sprn->r[spro->dst] = sprn->aluout;
				sprn->pc = (spro->pc + 1) & 0xFFFF; 
            case LHI:
                if (spro->dst != 0)
                    sprn->r[spro->dst] = sprn->aluout;
                sprn->pc = (spro->pc + 1) & 0xFFFF;                break;
                
            // --- Load Instruction ---
            case LD:
                // Sample the memory output and store it into the destination register.
                sprn->r[spro->dst] = llsim_mem_extract_dataout(sp->sram, 31, 0);
                sprn->pc = (spro->pc + 1) & 0xFFFF; //fail safe           
				break;
                
            // --- Store Instruction ---
            case ST:
                // ST writes R[src0] to memory at the address in R[src1].
                llsim_mem_set_datain(sp->sram, spro->alu0, 31, 0);
                llsim_mem_write(sp->sram, spro->aluout);
                sprn->pc = (spro->pc + 1) & 0xFFFF;                
				break;
                
            // --- Conditional Jump Instructions ---
			case JLT:
				if (sprn->aluout == 1) {
					sprn->r[7] = spro->pc;                   // Save current PC (original jump location)
					sprn->pc = (spro->immediate) & 0xFFFF;     // Jump to target
				}
				else {
					sprn->pc = (spro->pc + 1) & 0xFFFF;
				}
				break;
			case JLE:
				if (sprn->aluout == 1) {
					sprn->r[7] = spro->pc;                   // Save current PC (original jump location)
					sprn->pc = (spro->immediate) & 0xFFFF;     
				}
				else {
					sprn->pc = (spro->pc + 1) & 0xFFFF;
				}
				break;
			case JEQ:
				if (sprn->aluout == 1) {
					sprn->r[7] = spro->pc;                   // Save current PC (original jump location)
					sprn->pc = (spro->immediate) & 0xFFFF;     
				}
				else {
					sprn->pc = (spro->pc + 1) & 0xFFFF;
				}
				break;
			case JNE:
				if (sprn->aluout == 1) {
					sprn->r[7] = spro->pc;                   // Save current PC (original jump location)
					sprn->pc = (spro->immediate) & 0xFFFF;     
				}
				else {
					sprn->pc = (spro->pc + 1) & 0xFFFF;
				}
				break;
			// --- Indirect Jump ---
			case JIN:
				sprn->r[7] = spro->pc;                           // Save current PC (original jump location)
				sprn->pc = (spro->r[spro->alu0]) & 0xFFFF;        // Jump to address in reg[alu0]
				break;

            // --- Halt Instruction ---
            case HLT:
                dump_sram(sp);  // Dump SRAM contents on halt.
                sprn->ctl_state = CTL_STATE_IDLE;
                llsim_stop();
                break;
                
            // --- Default / Unrecognized Opcode ---
            default:
                sprn->pc = (spro->pc + 1) & 0xFFFF;                break;
        }
        // For all non-HLT instructions, transition back to FETCH0.
        if (spro->opcode != HLT)
            sprn->ctl_state = CTL_STATE_FETCH0;
        break;
    }

	
}
}
static void sp_run(llsim_unit_t *unit)
{
	sp_t *sp = (sp_t *) unit->private;

	if (llsim->reset) {
		sp_reset(sp);
		return;
	}

	sp->sram->read = 0;
	sp->sram->write = 0;

	sp_ctl(sp);
// <<< DMA FSM: carry over registers
	sp_registers_t *spro = sp->spro, *sprn = sp->sprn;
	sprn->dma_state = spro->dma_state;
	sprn->dma_src   = spro->dma_src;
	sprn->dma_dst   = spro->dma_dst;
	sprn->dma_len   = spro->dma_len;
	sprn->dma_count = spro->dma_count;

	// one cycle per DMA state
	switch (spro->dma_state) {
		case 0:  // IDLE
    		break;

		case 1:  // READ a word
    		llsim_mem_read(sp->sram, spro->dma_src + spro->dma_count);
    		sprn->dma_state = 2;
    		break;

		case 2:  // WRITE that word
    	{
        	// extract last‐cycle’s read result
        	int w = llsim_mem_extract_dataout(sp->sram, 31, 0);
        	llsim_mem_set_datain (sp->sram, w, 31, 0);
        	llsim_mem_write     (sp->sram, spro->dma_dst + spro->dma_count);

	        // advance or finish
    	    if (spro->dma_count + 1 < spro->dma_len) {
        	    sprn->dma_count = spro->dma_count + 1;
            	sprn->dma_state = 1;  // go back to READ
	        } else {
    	        sprn->dma_count = spro->dma_count + 1;
        	    sprn->dma_state = 0;  // done → IDLE
	        }
 	   }
	    break;

 	}
}

static void sp_generate_sram_memory_image(sp_t *sp, char *program_name)
{
        FILE *fp;
        int addr, i;

        fp = fopen(program_name, "r");
        if (fp == NULL) {
                printf("couldn't open file %s\n", program_name);
                exit(1);
        }
        addr = 0;
        while (addr < SP_SRAM_HEIGHT) {
                fscanf(fp, "%08x\n", &sp->memory_image[addr]);
                addr++;
                if (feof(fp))
                        break;
        }
	sp->memory_image_size = addr;

        fprintf(inst_trace_fp, "program %s loaded, %d lines\n", program_name, addr);

	for (i = 0; i < sp->memory_image_size; i++)
		llsim_mem_inject(sp->sram, i, sp->memory_image[i], 31, 0);
}

static void sp_register_all_registers(sp_t *sp)
{
	sp_registers_t *spro = sp->spro, *sprn = sp->sprn;

	// registers
	llsim_register_register("sp", "r_0", 32, 0, &spro->r[0], &sprn->r[0]);
	llsim_register_register("sp", "r_1", 32, 0, &spro->r[1], &sprn->r[1]);
	llsim_register_register("sp", "r_2", 32, 0, &spro->r[2], &sprn->r[2]);
	llsim_register_register("sp", "r_3", 32, 0, &spro->r[3], &sprn->r[3]);
	llsim_register_register("sp", "r_4", 32, 0, &spro->r[4], &sprn->r[4]);
	llsim_register_register("sp", "r_5", 32, 0, &spro->r[5], &sprn->r[5]);
	llsim_register_register("sp", "r_6", 32, 0, &spro->r[6], &sprn->r[6]);
	llsim_register_register("sp", "r_7", 32, 0, &spro->r[7], &sprn->r[7]);

	llsim_register_register("sp", "pc", 16, 0, &spro->pc, &sprn->pc);
	llsim_register_register("sp", "inst", 32, 0, &spro->inst, &sprn->inst);
	llsim_register_register("sp", "opcode", 5, 0, &spro->opcode, &sprn->opcode);
	llsim_register_register("sp", "dst", 3, 0, &spro->dst, &sprn->dst);
	llsim_register_register("sp", "src0", 3, 0, &spro->src0, &sprn->src0);
	llsim_register_register("sp", "src1", 3, 0, &spro->src1, &sprn->src1);
	llsim_register_register("sp", "alu0", 32, 0, &spro->alu0, &sprn->alu0);
	llsim_register_register("sp", "alu1", 32, 0, &spro->alu1, &sprn->alu1);
	llsim_register_register("sp", "aluout", 32, 0, &spro->aluout, &sprn->aluout);
	llsim_register_register("sp", "immediate", 32, 0, &spro->immediate, &sprn->immediate);
	llsim_register_register("sp", "cycle_counter", 32, 0, &spro->cycle_counter, &sprn->cycle_counter);
	llsim_register_register("sp", "ctl_state", 3, 0, &spro->ctl_state, &sprn->ctl_state);

	    // … existing llsim_register_register calls …

    // register DMA engine state & parameters
    llsim_register_register("sp", "dma_state", 2,  0, &spro->dma_state, &sprn->dma_state);
    llsim_register_register("sp", "dma_src",   16, 0, &spro->dma_src,   &sprn->dma_src);
    llsim_register_register("sp", "dma_dst",   16, 0, &spro->dma_dst,   &sprn->dma_dst);
    llsim_register_register("sp", "dma_len",   32, 0, &spro->dma_len,   &sprn->dma_len);
    llsim_register_register("sp", "dma_count", 32, 0, &spro->dma_count, &sprn->dma_count);

}

void sp_init(char *program_name)
{
	llsim_unit_t *llsim_sp_unit;
	llsim_unit_registers_t *llsim_ur;
	sp_t *sp;

	llsim_printf("initializing sp unit\n");

	inst_trace_fp = fopen("inst_trace.txt", "w");
	if (inst_trace_fp == NULL) {
		printf("couldn't open file inst_trace.txt\n");
		exit(1);
	}

	cycle_trace_fp = fopen("cycle_trace.txt", "w");
	if (cycle_trace_fp == NULL) {
		printf("couldn't open file cycle_trace.txt\n");
		exit(1);
	}

	llsim_sp_unit = llsim_register_unit("sp", sp_run);
	llsim_ur = llsim_allocate_registers(llsim_sp_unit, "sp_registers", sizeof(sp_registers_t));
	sp = llsim_malloc(sizeof(sp_t));
	llsim_sp_unit->private = sp;
	sp->spro = llsim_ur->old;
	sp->sprn = llsim_ur->new;

	sp->sram = llsim_allocate_memory(llsim_sp_unit, "sram", 32, SP_SRAM_HEIGHT, 0);
	sp_generate_sram_memory_image(sp, program_name);

	sp->start = 1;

	sp_register_all_registers(sp);
}
